...
class GrokStrategy(BaseStrategy):
    ...
    def _get_momentum_and_vol(self, state: MarketState) -> tuple[float | None, float | None]:
        ...
        recent_prices = prices[-self.vol_window_sec:]
        vol = np.std(np.diff(np.log(recent_prices))) if len(recent_prices) > 1 else 0.0
        return momentum, vol

    def _get_book_imbalance(self, market) -> float:
        """Order book imbalance: positive = YES pressure, negative = NO pressure"""
        # Using common attributes from Polymarket-style books in the backtester
    ...
    def on_tick(self, state: MarketState) -> list[Order]:
        ...
        # --------------------------------------------------
        # 1. Exit logic (for all tradable markets)
        # --------------------------------------------------
        ...
        # --------------------------------------------------
        if momentum is not None and state.cash >= self.min_cash_buffer:
            for slug, market in state.markets.items():
                ...
                if market.yes_ask + market.no_ask < 0.995:
        ...
        adj_min_momentum = self.min_momentum_base * (1 + 8 * vol)
        ...
        for slug, market in state.markets.items():
            ...
            if slug in self.entered_markets:
                continue
            if not (0.30 <= market.time_remaining_frac <= 0.92):
                continue
            if market.yes_ask <= 0 or market.no_ask <= 0:
                continue
            if (market.yes_book.spread > self.max_book_spread or
                ...
                continue
            if (getattr(market.yes_book, 'total_ask_size', 0) < 50 or
                getattr(market.no_book, 'total_ask_size', 0) < 50):
                continue

            imbalance = self._get_book_imbalance(market)

            # Bullish → Buy YES
            if momentum > adj_min_momentum and imbalance > 0.12:
                edge = (momentum / adj_min_momentum) + abs(imbalance) * 2.0
                time_factor = (1.0 - market.time_remaining_frac) * 1.5
                size = min(500.0, self.base_order_size * max(1.0, edge) * time_factor)
                size = max(20.0, size)
                ...
                if state.cash >= est_cost:
                    orders.append(
                        Order(
                            ...
                            side=Side.BUY,
                            size=size,
                            limit_price=market.yes_ask + 0.001,
                        )
                    )
                    self.entered_markets.add(slug)
            ...
            # Bearish → Buy NO
            elif momentum < -adj_min_momentum and imbalance < -0.12:
                ...
                if state.cash >= est_cost:
                    orders.append(
                        Order(
                            ...
                            side=Side.BUY,
                            size=size,
                            limit_price=market.no_ask + 0.001,
                        )
                    )
                    self.entered_markets.add(slug)
